...
const allEvents = await axios.get('http://localhost:4000/api/event/events-count-entity',{
  ...
  params: { entityRef: entityRef }
        ...
        if (userResponse.data.success) {
          ...
          toast.success("Welcome to Secretary dashboard");
        } else {
          toast.dismiss(toastId);
  ...
  const handleEventApproval = async (memberId:any) => {
    try {
                    ...
                    {eventsApproval.map((member, index) => (
                      <tr key={index} className="border-b px-2">
                        ...
                        <td className="py-3 px-6">
                          <button className="bg-[#F0F9FF] text-[#89868D] text-sm px-3 py-2 rounded-xl border border-[#0095FF]" onClick={() => handleEventApproval(member._id)} >
                            Approve
                          </button>
                        </td>
                      </tr>
