...
//   const [schedule, setSchedule] = useState<ScheduleEvent[]>([]);
  ...
  const [eventsApproval,setEventsApproval]=useState<Event | any>([]);
...
const signOut=()=>{
    localStorage.removeItem('culogin');
  ...
  useEffect(()=>{
    ...
    const checkLogin = () => {
        const loginStatus = localStorage.getItem('culoginToken');
        if (!loginStatus) {
            router.push('/login'); 
            return;
        }
    ...
    const fetchUser = async () => {
        ...
        try {
            const userResponse = await axios.get(`http://localhost:4000/api/studentRep/get/${id}`);
            ...
            const weeklyEvents=await axios.get('http://localhost:4000/api/event/weekly');
            const clubs=await axios.get('http://localhost:4000/api/clubRoutes/clubs');
            const deptSocieties=await axios.get('http://localhost:4000/api/deptSocieties/departmental-societies');
            const profsocieties=await axios.get('http://localhost:4000/api/proffSocieties/professional-societies');

            const communities=await axios.get('http://localhost:4000/api/communitiesRoutes/communities');

            const unApprovedEvents=await axios.get('http://localhost:4000/api/event/unapprovedEvents');
           ...
           if(unApprovedEvents && unApprovedEvents.data.success){
            ...
            // console.log("these are Weekly Events events : ",weeklyEvents);
            console.log("events : ",unApprovedEvents);
            // console.log("clubs: ",clubs);
            // console.log("deptsocieties",deptSocieties);
            // console.log("profsocieteis",profsocieties);
            ...
            if (userResponse.data.success && flagships.data.success && allEvents.data.success && monthlyEvents.data.success && weeklyEvents.data.success && clubs.data.success) {
                if (!user) { 
                    ...
                    toast.success("welcome to Student Representative dashboard");
                    ...
                    EntityData[1].data[2].value=weeklyEvents.data.events.length;
                ...
                toast.dismiss(toastId);
                toast.error("Some issue! reload please.");
            }
        } catch (error) {
            toast.dismiss(toastId);
            console.log("error : ",error);
            toast.error("Error! reload please.");
        }
    };
      ...
      <div className='flex w-screen min-h-screen'>
        <div className='sidebar  flex flex-col gap-6 w-1/5 min-h-screen'>
          <div className='flex flex-col items-start px-10 py-4'>
            ...
            <p className='text-3xl font-semibold'>{ user ?  user.name : "Unknown" }</p>
          </div>
          ...
          {entity.data.map((item, i) => (
            ...
            <div className='eventApproval-section shadow-md  rounded-2xl w-8/12 px-7 py-5'>
              ...
              <table className="w-full bg-white ">
                ...
                <tbody>
                  {
                    eventsApproval.map((event:any, index:any) => (
